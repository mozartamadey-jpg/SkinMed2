...
interface InjectionServicePageProps {
  hero: HeroProps;
        ...
        <main className="w-full mt-8 sm:mt-12 lg:mt-24 px-4 lg:px-8 max-w-[1400px] mx-auto">
          ...
          {pricing && pricing.length > 0 && (
            <section className="mb-32 lg:mb-48 max-w-4xl mx-auto px-4 lg:px-8">
              <div className="text-center mb-16">
                <h2 className="text-3xl lg:text-5xl font-light tracking-tight text-slate-900">
                  Стоимость <span className="font-serif italic text-slate-400">процедуры</span>
                </h2>
              </div>
              <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-sm">
                <ul className="divide-y divide-slate-100">
                  {pricing.map((item, i) => (
                    <li key={i} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 lg:p-8 hover:bg-slate-50 transition-colors">
                      <span className="text-lg text-slate-700 font-medium mb-2 sm:mb-0 max-w-lg">{item.name}</span>
                      <span className="text-xl text-blue-600 font-medium whitespace-nowrap">{item.price}</span>
                    </li>
                  ))}
                </ul>
              </div>
