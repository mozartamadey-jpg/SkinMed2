...
export default function PlasmaHairClient() {
  useEffect(() => {
  ...
  const advantages = [
    { icon: 'solar:shield-check-linear', title: '100% биосовместимость', desc: 'Используется собственная плазма пациента — нулевой риск аллергии и отторжения.' },
    { icon: 'solar:atom-linear', title: 'Факторы роста', desc: 'Обогащённая тромбоцитами плазма содержит концентрат факторов роста, запускающих регенерацию фолликулов.' },
    { icon: 'solar:bolt-linear', title: 'Быстрый результат', desc: 'Первые изменения заметны через 3–4 недели. Полный эффект — через 3–6 месяцев.' },
    { icon: 'solar:heart-linear', title: 'PRP-Cortexil', desc: 'Инновационный гипоаллергенный материал стимулирует рост клеток дермы и восстанавливает дефицит полезных веществ.' },
  ];

  const steps = [
    { title: 'Забор крови', desc: 'Из вены берётся 15–20 мл крови в стерильную пробирку с антикоагулянтом. Процедура абсолютно безболезненна.' },
    { title: 'Центрифугирование', desc: 'Кровь помещается в центрифугу на 10–15 минут. Происходит разделение на фракции — выделяется обогащённая тромбоцитами плазма (PRP).' },
    { title: 'Микроинъекции', desc: 'PRP вводится в кожу головы тончайшими иглами 30G в зоны истончения и выпадения. Процедура занимает 30–40 минут.' },
    { title: 'Результат', desc: 'Факторы роста активируют «спящие» фолликулы, улучшают кровоснабжение и запускают рост новых волос.' },
  ];

  const cases = [
    { before: '/images/services/trichology/plasma_before.png', after: '/images/services/trichology/plasma_after.png', label: 'Результат после курса плазмотерапии PRP-Cortexil — 6 сеансов, 4 месяца' },
  ];

  const prices = [
     { name: 'Плазмотерапия PRP для волос', price: 'от 6 400 ₽' },
     { name: 'Курс PRP для волос', price: 'от 22 000 ₽' },
     { name: 'PRP-Cortexil комплекс', price: 'от 30 000 ₽' },
     { name: 'Сочетанная терапия волос', price: 'от 8 000 ₽' },
     { name: 'Консультация трихолога', price: 'от 1 500 ₽' },];

  const faq = [
    { q: 'Что такое PRP-Cortexil?', a: 'Это методика обогащения собственной плазмы пациента тромбоцитарными факторами роста. Cortexil — инновационный гипоаллергенный материал, который усиливает регенеративное действие плазмы.' },
    { q: 'Сколько процедур нужно?', a: 'Стандартный курс — 4–6 процедур с интервалом 2–4 недели. Поддерживающие сеансы — 1 раз в 3–6 месяцев.' },
    { q: 'Есть ли восстановительный период?', a: 'Нет. После процедуры возможно лёгкое покраснение в зонах инъекций, которое проходит в течение нескольких часов. Можно вернуться к обычной жизни сразу.' },
    { q: 'Кому подходит плазмотерапия?', a: 'Процедура эффективна при андрогенетической и диффузной алопеции, телогеновом выпадении, истончении волос, восстановлении после стресса.' },
  ];
        ...
        <main className="w-full mt-12 sm:mt-16">
          <section className="mb-6 md:mb-10 reveal-up opacity-0">
            <div className="flex flex-wrap items-center gap-2 text-xs md:text-sm text-slate-500">
              ...
              <Icon icon="mdi:chevron-right" className="text-slate-400" />
              <span className="text-slate-700 font-medium">Плазмотерапия для волос</span>
            </div>
          </section>
          ...
          <section className="mb-20 md:mb-32 reveal-up opacity-0">
            <div className="relative rounded-[2rem] md:rounded-[3rem] overflow-hidden bg-[#050B14] min-h-[550px] md:min-h-[700px] flex items-center group shadow-[0_2rem_5rem_-1rem_rgba(0,0,0,0.3)]">
              <div className="absolute inset-0 z-0">
                <img src="/images/services/trichology/hero-plasma.png" alt="Плазмотерапия для волос" className="w-full h-full object-cover opacity-[0.35] mix-blend-overlay scale-105" />
              ...
              <div className="relative z-20 px-8 py-12 sm:px-12 md:px-16 w-full max-w-4xl">
                ...
                <h1 className="text-4xl md:text-6xl lg:text-[5.5rem] font-light tracking-[-0.04em] leading-[1.0] text-white mb-6 uppercase drop-shadow-2xl">
                  Плазмо-<br /><span className="font-serif italic text-[#60c2ff]/80">терапия</span>
                </h1>
                <p className="text-lg md:text-xl font-light text-slate-300 mb-8 max-w-2xl leading-relaxed">
                  Восстановление волос собственной обогащённой плазмой — без аллергии, без побочных эффектов. Инновационная методика PRP-Cortexil стимулирует рост новых волос из «спящих» фолликулов.
                </p>
            ...
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
              <div>
                ...
                <h2 className="text-[3rem] sm:text-5xl font-light text-slate-900 tracking-[-0.04em] leading-[1.0] mb-6">
                  Как работает <span className="font-serif italic text-slate-400">PRP</span>
                </h2>
                <p className="text-base sm:text-[17px] text-slate-600 font-light leading-relaxed mb-4">
                  <span className="font-medium text-[#60c2ff]">PRP-Cortexil</span> позволяет восстановить дефицит полезных веществ из собственной, обогащённой факторами роста плазмы пациента. Тромбоциты высвобождают более 30 видов факторов роста, которые стимулируют регенерацию.
                </p>
                <p className="text-base sm:text-[17px] text-slate-600 font-light leading-relaxed">
                  Инновационный гипоаллергенный материал для стимуляции роста клеток дермы. В отличие от искусственных препаратов, PRP не вызывает аллергических реакций и не имеет побочных эффектов.
                </p>
              </div>
              <div className="relative rounded-[2.5rem] overflow-hidden shadow-lg">
                <img src="/images/services/trichology/process-plasma.png" alt="Плазмотерапия PRP" className="w-full h-[400px] object-cover" />
          ...
          <section className="mb-32 lg:mb-48 relative z-10">
            <div className="text-center mb-16 reveal-up opacity-0">
              ...
              <h2 className="text-[3rem] sm:text-5xl lg:text-[5rem] font-light text-slate-900 tracking-[-0.04em] leading-[1.0] mb-6">Почему <span className="font-serif italic text-slate-400">PRP</span></h2>
            </div>
            <div className="grid sm:grid-cols-2 gap-6 stagger-container max-w-5xl mx-auto">
              {advantages.map((a, i) => (
          ...
          <section className="mb-32 lg:mb-48 relative z-10 max-w-6xl mx-auto">
            <div className="mb-16 reveal-up opacity-0 border-b border-slate-200/50 pb-8">
              ...
              <h2 className="text-[3rem] sm:text-5xl lg:text-[5rem] font-light text-slate-900 tracking-[-0.04em] leading-[1.0]">Как проходит <span className="font-serif italic text-slate-400">процедура</span></h2>
            </div>
          ...
          <section className="mb-32 lg:mb-48 relative z-10 reveal-up opacity-0">
            <div className="mb-16 text-center">
              ...
              <h2 className="text-[3rem] sm:text-5xl lg:text-[5rem] font-light text-slate-900 tracking-[-0.04em] leading-[1.0] mb-6">Результат <span className="font-serif italic text-slate-400">PRP</span></h2>
            </div>
          ...
          <section className="mb-32 lg:mb-48 max-w-6xl mx-auto w-full">
            <div className="w-full bg-white/60 backdrop-blur-xl border border-white/80 rounded-[3rem] p-10 md:p-16 shadow-[0_2rem_4rem_-1rem_rgba(0,0,0,0.03)] reveal-up opacity-0">
              <div className="mb-12 border-b border-slate-100 pb-6">
                <h2 className="text-[2.5rem] md:text-4xl font-light text-slate-900 tracking-tight">Стоимость <span className="font-serif italic text-slate-400">PRP</span></h2>
              </div>
            ...
            <div className="relative overflow-hidden rounded-[3rem] bg-[#050B14] p-10 md:p-16 lg:p-24 border border-white/10 shadow-[0_3rem_6rem_-1rem_rgba(0,0,0,0.4)]">
              ...
              <div className="relative z-10 flex flex-col items-center text-center max-w-4xl mx-auto">
                <h2 className="text-[3rem] sm:text-5xl lg:text-[5rem] font-light text-white tracking-[-0.04em] leading-[1.0] mb-6">
                  Восстановите волосы <span className="font-serif italic text-slate-400">естественным путём</span>
                </h2>
                <p className="text-slate-400 font-light text-lg mb-12 max-w-lg">Запишитесь на консультацию трихолога. Расскажем подробнее о PRP и подберём курс.</p>
