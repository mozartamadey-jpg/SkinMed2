...
export default function HairLossClient() {
  useEffect(() => {
  ...
  const causes = [
    { icon: 'solar:test-tube-linear', title: 'Гормональный дисбаланс', desc: 'Андрогенетическая алопеция — самая частая причина. У мужчин и женщин гормоны могут вызывать прогрессивное истончение волос.' },
    { icon: 'solar:heart-pulse-linear', title: 'Стресс', desc: 'Телогеновое выпадение на фоне хронического стресса — волосы массово переходят в фазу покоя и начинают выпадать через 2–3 месяца.' },
    { icon: 'solar:medical-kit-linear', title: 'Дефицит витаминов', desc: 'Нехватка железа, цинка, витаминов группы B, витамина D — частые причины диффузного выпадения волос.' },
    { icon: 'solar:dna-linear', title: 'Наследственность', desc: 'Генетическая предрасположенность определяет чувствительность фолликулов к дигидротестостерону.' },
    { icon: 'solar:shield-warning-linear', title: 'Аутоиммунные процессы', desc: 'Очаговая алопеция — иммунная система атакует волосяные фолликулы, вызывая выпадение округлыми очагами.' },
    { icon: 'solar:clipboard-text-linear', title: 'Заболевания щитовидной', desc: 'Гипо- и гипертиреоз напрямую влияют на цикл роста волос, вызывая их истончение и потерю.' },
  ];

  const methods = [
    { icon: 'solar:syringe-linear', title: 'Мезотерапия', desc: 'Микроинъекции витаминных коктейлей непосредственно в кожу головы. Восстановление обменных процессов, стимуляция роста новых волос, улучшение кератинизации.', link: '/services/mesotherapy-hair' },
    { icon: 'solar:atom-linear', title: 'Плазмотерапия PRP-Cortexil', desc: 'Введение собственной плазмы пациента, обогащённой факторами роста. Стимулирует регенерацию фолликулов и рост новых волос без аллергических реакций.', link: '/services/plasmotherapy-hair' },
    { icon: 'solar:bolt-linear', title: 'RF-лифтинг Vivace', desc: 'Микроигольчатый аппарат с радиочастотной энергией. Улучшение кровоснабжения, кислородного обмена, ускорение восстановления структуры волос.', link: '/services/trichology/rf-lifting-hair' },
  ];

  const cases = [
    { before: '/images/services/trichology/loss_before.png', after: '/images/services/trichology/loss_after.png', label: 'Результат лечения андрогенетической алопеции — курс плазмотерапии + мезотерапия, 6 месяцев' },
  ];

  const prices = [
     { name: 'Консультация трихолога', price: 'от 1 500 ₽' },
     { name: 'Трихоскопия', price: 'от 2 000 ₽' },
     { name: 'Мезотерапия волос', price: 'от 3 000 ₽' },
     { name: 'Плазмотерапия PRP', price: 'от 6 400 ₽' },
     { name: 'RF-лифтинг Vivace для волос', price: 'от 8 000 ₽' },
     { name: 'Комплексный курс лечения', price: 'от 25 000 ₽' },];

  const faq = [
    { q: 'Сколько длится лечение?', a: 'Минимальный курс — 6–8 процедур с интервалом 7–14 дней. Первые результаты видны через 2–3 месяца. Полный цикл восстановления — 6–12 месяцев.' },
    { q: 'Какие анализы нужно сдать?', a: 'Трихолог может назначить общий анализ крови, ферритин, цинк, витамин D, гормоны щитовидной железы, половые гормоны. Перечень определяется на консультации.' },
    { q: 'Болезненны ли процедуры?', a: 'Мезотерапия и PRP выполняются тончайшими иглами 30–32G. Дискомфорт минимален. При необходимости используем местную анестезию.' },
    { q: 'Можно ли совмещать с домашним уходом?', a: 'Обязательно. Трихолог назначит специализированные средства, подберёт витамины и скорректирует питание для максимального эффекта.' },
  ];
        ...
        <main className="w-full mt-12 sm:mt-16">
          <section className="mb-6 md:mb-10 reveal-up opacity-0">
            <div className="flex flex-wrap items-center gap-2 text-xs md:text-sm text-slate-500">
              ...
              <Icon icon="mdi:chevron-right" className="text-slate-400" />
              <span className="text-slate-700 font-medium">Лечение выпадения волос</span>
            </div>
          </section>
          ...
          <section className="mb-20 md:mb-32 reveal-up opacity-0">
            <div className="relative rounded-[2rem] md:rounded-[3rem] overflow-hidden bg-[#050B14] min-h-[550px] md:min-h-[700px] flex items-center group shadow-[0_2rem_5rem_-1rem_rgba(0,0,0,0.3)]">
              <div className="absolute inset-0 z-0">
                <img src="/images/services/trichology/hero-hair-loss.png" alt="Лечение выпадения волос" className="w-full h-full object-cover opacity-[0.35] mix-blend-overlay scale-105" />
              ...
              <div className="relative z-20 px-8 py-12 sm:px-12 md:px-16 w-full max-w-4xl">
                ...
                <h1 className="text-4xl md:text-6xl lg:text-[5.5rem] font-light tracking-[-0.04em] leading-[1.0] text-white mb-6 uppercase drop-shadow-2xl">
                  Лечение <br /><span className="font-serif italic text-[#60c2ff]/80">выпадения волос</span>
                </h1>
                <p className="text-lg md:text-xl font-light text-slate-300 mb-8 max-w-2xl leading-relaxed">
                  Определяем причину и подбираем комплексную терапию — мезотерапия, плазмотерапия PRP, RF-лифтинг. Экономия времени и ресурсов: обходимся без хирургии.
                </p>
          ...
          <section className="mb-32 lg:mb-48 relative z-10">
            <div className="text-center mb-16 reveal-up opacity-0">
              <span className="block text-xs sm:text-sm font-medium uppercase tracking-[0.2em] text-[#60c2ff] mb-4">— Причины</span>
              <h2 className="text-[3rem] sm:text-5xl lg:text-[5rem] font-light text-slate-900 tracking-[-0.04em] leading-[1.0] mb-6">Почему выпадают <span className="font-serif italic text-slate-400">волосы</span></h2>
            </div>
          ...
          <section className="mb-32 lg:mb-48 relative z-10 max-w-6xl mx-auto">
            <div className="mb-16 reveal-up opacity-0 border-b border-slate-200/50 pb-8">
              <span className="block text-xs sm:text-sm font-medium uppercase tracking-[0.2em] text-[#60c2ff] mb-4">— Лечение</span>
              <h2 className="text-[3rem] sm:text-5xl lg:text-[5rem] font-light text-slate-900 tracking-[-0.04em] leading-[1.0]">Методы <span className="font-serif italic text-slate-400">терапии</span></h2>
            </div>
            <div className="flex flex-col stagger-container">
              <EditorialList items={methods} />
            </div>
          </section>
                ...
                <div key={i} className="flex flex-col gap-4">
                  <div className="flex flex-col md:flex-row gap-4">
                    <div className="relative rounded-[2.5rem] overflow-hidden shadow-lg group flex-1">
                      <img src={c.before} alt="До лечения" className="w-full h-[350px] object-cover group-hover:scale-105 transition-transform duration-[1s]" />
                      <div className="absolute bottom-4 left-4 bg-white/80 backdrop-blur text-sm px-4 py-2 rounded-full font-medium">До</div>
                    </div>
                    <div className="relative rounded-[2.5rem] overflow-hidden shadow-lg group flex-1">
                      <img src={c.after} alt="После лечения" className="w-full h-[350px] object-cover group-hover:scale-105 transition-transform duration-[1s]" />
          ...
          <section className="mb-32 lg:mb-48 max-w-6xl mx-auto w-full">
            <div className="w-full bg-white/60 backdrop-blur-xl border border-white/80 rounded-[3rem] p-10 md:p-16 shadow-[0_2rem_4rem_-1rem_rgba(0,0,0,0.03)] reveal-up opacity-0">
              <div className="mb-12 border-b border-slate-100 pb-6">
                <h2 className="text-[2.5rem] md:text-4xl font-light text-slate-900 tracking-tight">Стоимость <span className="font-serif italic text-slate-400">лечения</span></h2>
              </div>
              ...
              <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12 max-w-6xl mx-auto">
                <div className="flex-1 text-center lg:text-left">
                  <h2 className="text-[3rem] sm:text-5xl lg:text-[5rem] font-light text-white tracking-[-0.04em] leading-[1.0] mb-6">
                    Остановите выпадение <br /><span className="font-serif italic text-slate-400">сегодня</span>
                  </h2>
                  <p className="text-slate-400 font-light text-lg mb-12 max-w-lg mx-auto lg:mx-0">Запишитесь на диагностику. Трихолог определит причину и составит индивидуальный план лечения.</p>
