...
export default function PostAcneClient() {
  ...
  const indications = [
    { title: "Атрофические рубцы", desc: "Ямки и впадинки на коже («сколотые», «прямоугольные» и «закругленные» рубцы), оставшиеся после глубоких воспалений.", icon: "solar:adhesive-plaster-linear" },
    { title: "Застойные пятна", desc: "Синюшные, красные или коричневые пятна (гиперпигментация), сохраняющиеся месяцами после заживления прыщей.", icon: "solar:danger-triangle-linear" },
    { title: "Неровный рельеф", desc: "Общая бугристость и визуальная «рыхлость» или «усталость» кожи лица.", icon: "solar:layers-minimalistic-linear" },
    { title: "Расширенные поры", desc: "«Кратерообразные» поры, потерявшие свою эластичность из-за перенесенного воспаления.", icon: "solar:stars-linear" },
    { title: "Скрытые микрорубцы", desc: "Снижение тургора и плотности кожи в зонах высыпаний (щеки, подбородок, виски).", icon: "solar:health-linear" },
    { title: "Гипертрофические шрамы", desc: "Плотные, выпуклые, возвышающиеся над поверхностью кожи рубцы на месте бывших узлов.", icon: "solar:shield-check-linear" },
  ];

  const advantages = [
    { title: "Двойное действие", desc: "Лазер не только выпаривает старую рубцовую ткань, но и запускает мощный синтез нового коллагена, который «выталкивает» ямки изнутри." },
    { title: "Контролируемая глубина", desc: "Врач-косметолог с точностью до микрон настраивает глубину проникновения луча для каждого типа рубца (Ice Pick, Boxcar, Rolling)." },
    { title: "Фракционная технология", desc: "Луч подается в виде сетки. Нетронутые клетки между микрозонами повреждения обеспечивают сверхбыструю регенерацию." },
    { title: "Стойкий результат", desc: "Ремоделирование кожи продолжается в течение 3–6 месяцев после сеанса, обеспечивая нарастающий и постоянный эффект." }
  ];

  const steps = [
    { title: "Консультация", desc: "Оценка типа рубцов (сколотые, прямоугольные или закругленные) и подбор параметров фракционного аблятивного лазера." },
    { title: "Анестезия", desc: "Нанесение мощного аппликационного крема-анестетика (под пленку на 30–40 минут) для комфорта процедуры." },
    { title: "Лазерная шлифовка", desc: "Обработка зон постакне. Вы можете чувствовать интенсивное тепло и легкое покалывание." },
    { title: "Охлаждение и защита", desc: "Нанесение успокаивающих регенерирующих мазей. Врач выдает подробную инструкцию по домашнему уходу." }
  ];

  const aftercare = [
    { title: "Интенсивный уход", desc: "Наносите рекомендованный врачом заживляющий крем (Бепантен/Пантенол) плотным слоем каждые 2-3 часа.", icon: "solar:medical-kit-linear" },
    { title: "Отказ от умывания", desc: "Первые сутки нельзя умываться водой. Затем только мягкое орошение антисептиком без трения.", icon: "solar:waterdrop-linear" },
    { title: "Без тепла и спорта", desc: "Исключите бани, сауны, интенсивный спорт и алкоголь на 2 недели, чтобы избежать сильного отека.", icon: "solar:temperature-linear" },
    { title: "Защита SPF 50+", desc: "После схождения корочек (5-7 дней) использование крема с SPF 50 обязательно в течение месяца.", icon: "solar:sun-2-linear" }
  ];

  const faqData = [
    { q: "Сколько процедур нужно для полного избавления от рубцов?", a: "Постакне — сложная проблема. Полностью «стереть» глубокие сколотые рубцы невозможно, но можно сгладить рельеф на 70–80%. В среднем требуется от 2 до 4 процедур фракционной шлифовки с интервалом в 1–2 месяца." },
    { q: "Можно ли делать шлифовку, если еще есть воспаления?", a: "Нет. Лазерная шлифовка проводится только на стадии стойкой ремиссии, когда нет глубоких гнойных элементов. Если у вас активное акне, врач сначала назначит медикаментозное лечение (например, системные ретиноиды или неодимовый лазер)." },
    { q: "Как выглядит лицо после шлифовки?", a: "Первые 2–3 дня наблюдается сильный отек и покраснение (эффект «ошпаренного лица»), на коже видна «марлевая сеточка» от микроповреждений. Затем сеточка темнеет, превращается в мелкие корочки и отшелушивается (5–7 дней). Через 7–10 дней вы можете выходить на работу, используя легкий тональный крем." },
    { q: "Когда будет виден окончательный результат?", a: "Первичное выравнивание рельефа заметно через 2–3 недели после схождения корочек. Однако окончательный результат оценивается через 3–6 месяцев — именно столько времени требуется коже для выработки нового коллагена и поднятия дна рубца." }
  ];
          ...
          <main className="w-full mt-12 sm:mt-16 flex flex-col gap-20 md:gap-32 pb-24">
            ...
            <section className="mb-[-2rem] reveal-up opacity-0">
              <div className="flex flex-wrap items-center gap-2 text-sm sm:text-base text-slate-500 font-light">
                ...
                <Icon icon="mdi:chevron-right" className="text-slate-400" />
                <span className="text-slate-700 font-medium">Шлифовка рубцов постакне</span>
              </div>
            </section>
            ...
            <section className="reveal-up opacity-0 scroll-glow-item group">
              <div className="bg-[#050B14] rounded-[2rem] md:rounded-[3rem] p-8 md:py-16 md:px-20 min-h-[550px] md:min-h-[700px] relative overflow-hidden shadow-[0_2rem_5rem_-1rem_rgba(0,0,0,0.3)] flex items-center border border-white/10 group-[.mobile-glow-active]:shadow-[#60c2ff]/30 transition-all duration-700">
                <div className="absolute inset-0 z-0 overflow-hidden">
                  ...
                  <ServiceImage src="/images/post-acne-hero.png" alt="Шлифовка рубцов постакне" sizes="100vw" priority className="w-full h-full object-cover opacity-80 mix-blend-overlay scale-[1.02] md:group-hover:scale-[1.04] transition-transform duration-[2s]" />
                ...
                <div className="relative z-20 w-full max-w-4xl mt-auto md:mt-0 pt-32 md:pt-0">
                  <div className="inline-flex items-center gap-3 bg-[#60c2ff]/10 backdrop-blur-md text-[#60c2ff] px-5 py-2.5 rounded-full border border-[#60c2ff]/20 shadow-sm mb-6">
                    ...
                    <span className="text-xs sm:text-sm font-semibold uppercase tracking-widest text-white">Выравнивание рельефа кожи</span>
                  </div>
                  ...
                  <h1 className="text-4xl md:text-6xl lg:text-[6rem] font-light text-white leading-[1.0] uppercase tracking-[-0.04em] drop-shadow-2xl mb-6">
                    Шлифовка <br/> <span className="font-serif italic text-[#60c2ff]/80">постакне</span>
                  </h1>
                  
                  <p className="text-lg md:text-xl font-light text-slate-300 leading-relaxed max-w-2xl mb-8">
                    Устраните «ямки» от прыщей, неровный рельеф и пятна с помощью фракционного CO2 и эрбиевого лазера. Идеально гладкая кожа без следов воспалений.
                  </p>
            ...
            <section className="reveal-up opacity-0 flex flex-col items-center max-w-7xl mx-auto w-full relative z-10">
              <div className="text-center mb-16">
                ...
                <h2 className="text-[3rem] sm:text-5xl lg:text-[5rem] font-light text-slate-900 tracking-[-0.04em] leading-[1.0] mb-6">
                  Когда необходима <span className="font-serif italic text-slate-400">шлифовка</span>
                </h2>
              </div>
            ...
            <section className="reveal-up opacity-0 flex flex-col lg:flex-row gap-12 lg:gap-20 items-center bg-white/60 backdrop-blur-xl border border-white/80 rounded-[3rem] p-8 md:p-12 lg:p-16 shadow-[0_1rem_3rem_-1rem_rgba(0,0,0,0.05)] max-w-7xl mx-auto scroll-glow-item group">
               <div className="w-full lg:w-1/2 flex flex-col items-start gap-6">
                  <span className="text-xs uppercase tracking-widest text-[#60c2ff] font-bold block">— Фракционный лазер</span>
                  <h2 className="text-[2.5rem] md:text-5xl font-light text-slate-900 leading-tight">
                     Сглаживание изъянов <br/>из-<span className="font-serif italic text-[#60c2ff]">внутри</span>
                  </h2>
                  <p className="text-lg md:text-xl text-slate-600 font-light leading-relaxed mt-4">
                     Рубцы постакне образуются из-за дефицита или переизбытка коллагена в местах сильных гнойных воспалений. Никакие кремы или пилинги не способны проникнуть на эту глубину дермы.
                  </p>
                  <p className="text-lg md:text-xl text-slate-600 font-light leading-relaxed">
                     CO2 лазер генерирует микролучи, которые по принципу «решета» (фракционно) выпаривают рубцовую ткань. Здоровые участки между микроканалами начинают интенсивно вырабатывать новый коллаген и гиалуроновую кислоту. Эта новая ткань буквально выталкивает ямку наружу, выравнивая рельеф лица.
                  </p>
               </div>
               <div className="w-full lg:w-1/2 relative rounded-[2.5rem] overflow-hidden aspect-[4/3] lg:h-[500px] shadow-2xl">
                  <ServiceImage src="/images/post-acne-process.png" alt="Фракционная лазерная шлифовка лица" sizes="(max-width: 1024px) 100vw, 50vw" className="w-full h-full object-cover md:group-hover:scale-[1.03] transition-transform duration-700" />
               </div>
            </section>
            ...
            <section className="reveal-up opacity-0 relative z-10 w-full max-w-6xl mx-auto scroll-glow-item group">
              <div className="bg-gradient-to-br from-[#FAFAFA] to-white rounded-[3rem] p-8 md:p-16 xl:p-20 shadow-[0_1rem_4rem_-1rem_rgba(0,0,0,0.05)] border border-white relative overflow-hidden transition-all duration-700 group-[.mobile-glow-active]:shadow-[#60c2ff]/20">
                ...
                <h2 className="text-[2.5rem] md:text-5xl lg:text-[4rem] font-light text-slate-900 tracking-tight leading-tight mb-16 text-center relative z-10">
                  Этапы <span className="font-serif italic text-[#60c2ff]">шлифовки</span>
                </h2>
              ...
              <div className="w-full bg-white/60 backdrop-blur-xl border border-white/80 rounded-[3rem] p-10 md:p-16 shadow-[0_2rem_4rem_-1rem_rgba(0,0,0,0.03)] reveal-up opacity-0">
                ...
                <div className="flex flex-col divide-y divide-slate-100">
                  <div className="flex flex-col sm:flex-row justify-between sm:items-center py-6 group hover:px-4 transition-all duration-300 gap-2">
                    <span className="text-lg lg:text-xl font-medium text-slate-800 group-hover:text-[#60c2ff] transition-colors">Лазерная шлифовка лица (полностью)</span>
                    <span className="text-xl lg:text-2xl font-light text-slate-900 bg-slate-50 px-5 py-2.5 rounded-full border border-slate-100 shrink-0">от 12 000 ₽</span>
                  </div>
                  <div className="flex flex-col sm:flex-row justify-between sm:items-center py-6 group hover:px-4 transition-all duration-300 gap-2">
                    <span className="text-lg lg:text-xl font-medium text-slate-800 group-hover:text-[#60c2ff] transition-colors">Лазерная шлифовка щек (зона постакне)</span>
                    <span className="text-xl lg:text-2xl font-light text-slate-900 bg-slate-50 px-5 py-2.5 rounded-full border border-slate-100 shrink-0">от 6 000 ₽</span>
                  </div>
                  <div className="flex flex-col sm:flex-row justify-between sm:items-center py-6 group hover:px-4 transition-all duration-300 gap-2">
                    <span className="text-lg lg:text-xl font-medium text-slate-800 group-hover:text-[#60c2ff] transition-colors">Локальная шлифовка (отдельные рубцы, 1 кв. см)</span>
            ...
            <section className="reveal-up opacity-0 flex flex-col lg:flex-row gap-12 lg:gap-20 max-w-7xl mx-auto w-full items-start">
               <div className="w-full lg:w-1/3 sticky top-32">
                 ...
                 <p className="text-lg text-slate-500 font-light mb-8">Собрали для вас ответы на часто задаваемые вопросы о лечении постакне.</p>
                 <button 
                  onClick={() => typeof window !== 'undefined' && window.dispatchEvent(new Event('open-booking-modal'))}
                 ...
                 <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12 max-w-6xl mx-auto">
                   <div className="flex-1 text-center lg:text-left">
                     <h2 className="text-[3rem] sm:text-5xl lg:text-[5rem] font-light text-white tracking-[-0.04em] leading-[1.0] mb-6">
                       Гладкая кожа <br />
                       <span className="font-serif italic text-slate-400">без следов</span> акне
                     </h2>
                     <p className="text-slate-400 font-light text-lg lg:text-xl mb-12 max-w-lg mx-auto lg:mx-0 leading-relaxed">
                       Запишитесь на консультацию к врачу-косметологу. Мы подберем эффективный протокол лазерной шлифовки, чтобы вернуть вашей коже ровный рельеф и здоровый цвет.
                     </p>
